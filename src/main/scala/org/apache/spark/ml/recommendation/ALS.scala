...
object ALS extends DefaultParamsReadable[ALS] with Logging {
  ...
  private def makeBlocks[ID: ClassTag](
	...
	*/
	...
	private def computeFactors[ID](
		...
		try {
			if (implicitPrefs) {
				computeFactorsOriginal(srcFactorBlocks, srcOutBlocks, dstInBlocks, rank, regParam, srcEncoder, implicitPrefs, alpha, solver)
			} else {
				...
				dstInBlocks.join(merged).mapValues {
					...
					val nRqsts = new ArrayBuffer[java.lang.Integer]()

					var size = 0

					var chunkRequestsNo = 0

					val maxSize:Int = 4 * (Integer.MAX_VALUE / 4 / rank);

					var j = 0
					while (j < dstIds.length) {
						val tempSize = srcPtrs(j + 1) - srcPtrs(j)
						...
						if (tempSize < 800) {
							...
							if (newSize < maxSize) {
								...
								requests += als
								buffers += Seq(matrixTranspose, vector, offsets, lambdas, result)

								cnt = 0;

								chunkRequestsNo += 1
								indxs += indexes.toArray
								nRqsts += numRequests
								indexes.clear
								indexes += j
								size = tempSize
					...
					for (c <- 0 until chunkRequestsNo) {
						val als = requests(c)
						val matrixTranspose = buffers(c)(0)
   ...
   * @return dst factors
   */
  private def computeFactorsOriginal[ID](
      srcFactorBlocks: RDD[(Int, FactorBlock)],
      srcOutBlocks: RDD[(Int, OutBlock)],
      dstInBlocks: RDD[(Int, InBlock[ID])],
      rank: Int,
