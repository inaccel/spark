...
// InAccel (BEGIN)
import org.apache.spark.ml.inaccel.classification.{LogisticRegression => InAccelLogisticRegression}
import org.apache.spark.ml.inaccel.util.{DatasetUtils => InAccelDatasetUtils, MetadataUtils => InAccelMetadataUtils}
  ...
  private[spark] def setInitialModel(model: LogisticRegressionModel): this.type = {
	...
	override protected[spark] def train(dataset: Dataset[_]): LogisticRegressionModel = {
		try {
			...
			val instances: RDD[MLlibLabeledPoint] = InAccelDatasetUtils.toLabeledPointRDD(dataset, $(labelCol), $(featuresCol))

			val algo = new InAccelLogisticRegression()
				.setNumClasses(InAccelMetadataUtils.getNumClasses(dataset, $(labelCol)))
				.setNumFeatures(InAccelMetadataUtils.getNumFeatures(dataset, $(featuresCol)))
				.setAlpha($(elasticNetParam))
				.setGamma(0.9)
				.setMaxIter($(maxIter))
				.setTol($(tol))

			val model = copyValues(algo.run(instances).setParent(this))
			...
			case e: Exception => {
				...
				trainOriginal(dataset)
			}
		}
	}
	// InAccel (END)

  protected[spark] def trainOriginal(dataset: Dataset[_]): LogisticRegressionModel = {
