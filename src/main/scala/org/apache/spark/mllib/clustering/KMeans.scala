...
class KMeans private (
  ...
  private[spark] def runWithWeight(
	...
	// InAccel (BEGIN)
	private def runAlgorithmWithWeight(data: RDD[VectorWithNorm], instr: Option[Instrumentation]): KMeansModel = {
		try{
			require(k <= 16383, "! Unsupported number of Clusters " + k + ". Falling back.")
			...
			val centers = initialModel match {
				case Some(kMeansCenters) => kMeansCenters.clusterCenters.map(new VectorWithNorm(_))
				case None =>
			...
			println("- Entering InAccel FPGA acceleration layer -")
			println("-                  KMeans                  -")

			val numClusters = k
			val numFeatures = centers.head.vector.size

			if (numClusters < 208) require(numFeatures <= 1023, "! Unsupported number of Features " + numFeatures + ". Falling back.")
			else require(numFeatures <= 8191, "! Unsupported number of Features " + numFeatures + ". Falling back.")
			...
			while (iteration < maxIterations && !converged) {
				ConsoleProgressBar.show(iteration, maxIterations)

				val sums_counts = fpga_data.map(data => InAccelRDD.sums_counts(data, centers.map(_.vector))).reduce((a, b) => a.zip(b).map{case (x, y) => x + y})

				var newCenters = ArrayBuffer[VectorWithNorm]()

				for (k <- 0 until numClusters) {
					if (sums_counts(k * (numFeatures + 1) + numFeatures).toInt != 0) {
						val centersArray = new Array[Double](numFeatures)
						for (j <- 0 until numFeatures) {
							centersArray(j) = (sums_counts(k * (numFeatures + 1) + j) / sums_counts(k * (numFeatures + 1) + numFeatures)).toDouble
						}
						newCenters += new VectorWithNorm(centersArray)
					}
				}
				...
				var cnt:Int = 0
				for (k <- 0 until numClusters) {
					if (converged && !distanceMeasureInstance.isCenterConverged(centers(k), newCenters(k), epsilon)) {
						converged = false
					}
			...
			println("- Leaving InAccel FPGA acceleration layer -")

			new KMeansModel(centers.map(_.vector), distanceMeasure, 0.0, iteration)
		} catch {
			case e: Exception => {
				...
				runAlgorithmWithWeightOriginal(data, instr)
			}
		}
	}
	// InAccel (END)
   ...
   */
  private def runAlgorithmWithWeightOriginal(
