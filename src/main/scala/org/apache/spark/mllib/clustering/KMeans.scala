...
class KMeans private (
  ...
  private[spark] def run(
	...
	// InAccel (BEGIN)
	private def runAlgorithm(data: RDD[VectorWithNorm], instr: Option[Instrumentation]): KMeansModel = {
		try{
			...
			case e: Exception => {
				...
				runAlgorithmOriginal(data, instr)
			}
		}
	}
	// InAccel (END)
   ...
   */
  private def runAlgorithmOriginal(
